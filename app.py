    ...
    if request.headers.get('Authorization') == API_KEY:
        if int(current_user.id) == int(shop_id):
            ...
            if request.method == 'POST':
                ...
                return render_template('editMenuItem.html', shop=shop, item=item, main_cats=main_cats, sub_cat=sub_cat)
        else:
            return Response("Not Authorised")
    return API_KEY_ERROR


@app.route('/editMyShop/<int:shop_id>', methods=['GET', 'POST'])
# Task 1: Create route for newShopItem function here
def edit_shop_temp(shop_id):
    if request.headers.get('Authorization') == API_KEY:
        if int(current_user.id) == int(shop_id):
            shop = db.session.query(models.Shop).filter_by(id=shop_id).one()
            if request.method == 'POST':
...
def make_order():
    if request.headers.get('Authorization') == API_KEY:
        if request.method == 'POST':
            ...
            alert = {'Order Sent': 'Hello world', 'notification': {'title': "Order Sent",
                                                                        'body': "Your Order have been sent to " +
                                                                                shop.shop_name +
                                                                                " waiting for approval"}}
            if order is None:
                order = models.Orders(user=user, item=item, quantity=quantity)
                client.send(user.device_token, alert)
